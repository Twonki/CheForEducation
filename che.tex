...
remote environment. 
Instead of developing on a local machine, a remote development-server is accessed via a webbrowser 
...
With that in mind,an IDE is just a toolstack to make things (a lot) more comfortable – but it also has requirements such as the JDK, the .Jars needed in the classpath etc. as they are only utilized by the IDE.
...
This  will grant access to autocompletion and other features, but will miss Che specific helpers the web-IDE offers. 
...
Typically, after making the first iterations of the software on a local machine, dockering it will yield to 100 problems at once, not only being frustrating but taking tons of time. 

With Che, this kind of Big-Bang-Migration is not possible, as from the very first step its working inside of a docker-container.
...
The combination of a quick setup and the usability of every enhancement makes Che especially interesting to recruit troubleshooters into projects, which solve some of the very specialiced tasks.  
